  ...
  // check pot minimum sensor value
   if (voltage >= lapV) {
    for (int i = 0; i < 10; i++) {
     ...
     Serial.println("comprimento inicial" );
     Serial.println(rawData(cI));
     Serial.println("diâmetro inicial" );
     Serial.println(rawData(dI));
     delay(100);
    }
  ...
  // check pot minimum sensor value
   if (voltage >= lapV) {
    for (int i = 0; i < 20; i++) {
     ...
     cI = cI + cE;
     dapI = cI / Pi;
     //dtostrf(dapI,7, 2, dapInitial);
     Serial.println("===== MEANSUREMENT MODE =====");
     Serial.print("DAP Initial (cm) = ");
     Serial.println(dapI);
     Serial.println("voltage = " );
     Serial.println(rawData(voltage));
     delay(100);
...
// growth mode
 while (growth == true) {
  ...
  cL = cI + cE;
  dapL = cL / Pi;
  //dtostrf(dapL,7, 2, dapLast);
  if (dapL > dapI) {
    ICAd = dapL - dapI;
    //dtostrf(ICAd,7, 1, ICAdiameter);
    Serial.println("===== GROWTH STATUS =====");
    Serial.println("DAP Last (cm) = " );
    Serial.println(rawData(dapL));
    Serial.println("ICAd (cm) = " );
    Serial.println(rawData(ICAd));
    Serial.println("voltagem (volts) = " );
    Serial.println(rawData(voltage));
    Serial.println("comprimento (cm) = " );
    Serial.println(rawData(cL));
    dataLogger = String(rawData(dapI)) + "," + String(rawData(dapL)) + "," + String(rawData(ICAd));
